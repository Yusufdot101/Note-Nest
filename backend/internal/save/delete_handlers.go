...
func (h *saveHandler) unsaveNote(w http.ResponseWriter, r *http.Request) {
	...
	err = h.svc.unsaveNote(userID, noteID)
	if err != nil {
		switch {
	...
	err = utilities.WriteJSON(w, utilities.Message{"message": "unsaved note"}, http.StatusOK)
	if err != nil {
		customerrors.ServerErrorResponse(w, err)
		return
	}
