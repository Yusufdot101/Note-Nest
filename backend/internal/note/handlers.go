...
func (h *NoteHandler) newNote(w http.ResponseWriter, r *http.Request) {
	...
	n, err := h.svc.newNote(v, userID, projectID, input.Title, input.Content, input.Visibility, input.Color)
	if err != nil {
		switch {
	...
	err = utilities.WriteJSON(w, utilities.Message{"note": n}, http.StatusCreated)
	if err != nil {
		custom_errors.ServerErrorResponse(w, err)
		return
	}
